...
const CourseIdPage = async ({ params }: { params: Promise<{ courseId: string }> }) => {
    ...
    const [course, modules] = await Promise.all([
        ...
        currentUser?.role === 'STUDENT'
            ? db.courseModule.findMany({ where: { courseId, isPublished: true }, orderBy: { index: 'asc' } })
            : db.courseModule.findMany({ where: { courseId }, orderBy: { index: 'asc' } }),
    ]);
    ...
    const completedModules = modules.filter(module => progress.some(p => p.courseModuleId === module.id && !p.isDone)).length;
