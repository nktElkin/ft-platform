...
const ModulesTable = ({ modules, currentUser}: ModulesTableProps) => {
  ...
  return (<>
    <Accordion type="single" collapsible aria-label="Course study modules" role="region">
      {modules.map((module: CourseModule) => {
        return (
          <AccordionItem role="presentation" value={module?.id} key={module?.id}>
            <AccordionTrigger role="button" aria-controls={`content-${module?.id}`} id={`trigger-${module?.id}`}>
              <span className="sr-only">{!module.isPublished ? 'draft' : 'published'} module</span>
              <span>
                {currentUser?.role !== 'STUDENT' &&
                  <span className="inline-block mr-3">
                    <Indicator state={module.isPublished} data={['published', 'draft']} /></span>}
