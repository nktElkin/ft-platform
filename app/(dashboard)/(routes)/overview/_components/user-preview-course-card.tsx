...
const CoursePreviewCard = ({ course, courseCategory = 'Uncategorized' }: CoursePreviewCardProps) => {
    return (
        <Link href={`ovewview/courses/${course.id}`} className="h-full block">
                <div aria-roledescription='course preview-card' className='h-full w-full rounded-md border border-input bg-transparent p-3 text-base shadow-sm sm:text-sm relative hover:shadow-md transition duration-300 ease-in-out  flex flex-col justify-around'>
                    <div>
                        ...
                        <div className='text-start'>
                            ...
                            <p className="text-sm text-zinc-500 mb-2">{!course?.description ? <>Hmmm, not alot<br/>Click on and find oit more info!</> : course?.description.length > 90 ? `${course?.description?.slice(0, 90)}...` : `${!course?.description }`}</p>
                        </div>
                        <IndicatorsBlock type='course' object={course} categoryName={courseCategory} />
                    </div>
                </div>
        </Link>
    );
