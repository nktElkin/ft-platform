...
const TextEditSection = ({ initialValue, courseId, moduleId, time = 2000 }: TextEditSectionProps) => {
    ...
    return (
        <form onSubmit={handleManualUpload} className='space-y-2' aria-label="Markdown editor form"
            role="form">
            ...
            <MDEditor
                ...
                onChange={setValue}
            />
            <Button disabled={isLoading || value === lastSavedValue}
                aria-busy={isLoading}
                aria-label={isLoading ? "Saving changes" : "Save changes"} type='submit'><LoadingSpinner state={isLoading ? 'loading' : ''} /> {isLoading ? <>Loading</> : 'Upload'}</Button>
        </form>
