...
const getPathWithNext = (pathname: string, matchPattern: string): string => {
  if (
    ...
    (pathname.includes("tutor") || pathname.includes("student"))
  )
    return "/overview";
...
const NavigationBreadcrumbs = () => {
  ...
  const [hiddenItems, visibleItems] = setItemsArray(
    ITEMS_TO_DISPLAY,
        ...
        <BreadcrumbList>
                  ...
                  <Drawer open={open} onOpenChange={setOpen}>
                    ...
                    <DrawerContent>
                      <DrawerHeader className="text-left">
                        ...
                        <DrawerDescription>Select a page</DrawerDescription>
                      </DrawerHeader>
          ...
          {visibleItems.length > 1
            ? visibleItems.map((item, index) => (
                <BreadcrumbItem
                  className={`hover:underline hover:text-zinc-700 ${item?.href === pathname ? "text-zinc-700" : ""}`}
                  key={index}
                >
                  {item.href ? (
                    <>
