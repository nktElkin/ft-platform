...
const PreviewCard = ({ object, variant, courseCategory, courseId, hover }: PreviewCardProps) => {
    ...
    return (
        <div aria-roledescription={`${variant} preview-card`}
            ...
            {variant === 'course' &&
                <>
                    {object?.wallpaperUrl &&
                    <img src={`${object?.wallpaperUrl}`} alt="object Image" className="w-full h-40 object-cover rounded-t-md mb-3" />}
                    <div className='text-start'>
                        <h3 className={`text-lg font-semibold ${variant === 'course' && 'mb-2'}`}>{title}</h3>
                        <p className="text-sm text-zinc-500 mb-2">{description.length > 120 ? `${description.slice(0, 120)}...` : `${description}`}</p>
                        <IndicatorsBlock type={variant} object={object} categoryName={courseCategory} />
                    </div>
            ...
            {variant === 'module' &&
                <>
                    <div className='text-start'>
                        ...
                        <Link href={`/tutor/edit-course/${courseId}/edit-module/${object?.id}`}>
                            ...
                            <div className="flex flex-row items-center">
                                <IndicatorsBlock type={variant} object={object} categoryName={courseCategory} />
                                <RemoveCourseModuleBtn courseId={object?.courseId} moduleId={object?.id} />
                            </div>
                        </div>
                    </div>
                </>}
